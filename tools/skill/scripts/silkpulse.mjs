...
async function resolveDeviceId(idOrPrefix) {
  ...
  if (matched.length === 0) {
    console.error(
      `未找到设备: ${idOrPrefix}（在线设备: ${devices.map((d) => d.id).join(", ") || "无"}）`,
    );
    process.exit(1);
  }
  if (matched.length > 1) {
    console.error(
      `设备 id 前缀 "${idOrPrefix}" 匹配到多个: ${matched.map((d) => d.id).join(", ")}`,
    );
    process.exit(1);
  }
  return matched[0].id;
...
async function main() {
  switch (cmd) {
    case "devices": {
      ...
      if (devices.length === 0) {
        console.log(
          '（暂无在线设备。接入方式：在目标页面注入 <script src="' +
            SERVER +
            '/sdk.js"></script>）',
        );
      ...
      for (const d of sorted) {
        ...
        console.log(
          `       类型: ${d.deviceType ?? "unknown"} · ${d.viewportWidth}×${d.viewportHeight}${onlineDur}`,
        );
    ...
    case "exec": {
      ...
      if (!code) {
        if (process.stdin.isTTY) {
          console.error(
            '缺少要执行的 code（可在命令行传入，或通过管道：echo "..." | silkpulse exec <id>）',
          );
          process.exit(1);
        }
    ...
    case "network": {
      ...
      for (const e of display) {
        ...
        if (e.protocol === "ws") {
          const stateName =
            ["CONNECTING", "OPEN", "CLOSING", "CLOSED"][e.wsState] || String(e.status);
          const frames = e.frames || [];
          const sendN = frames.filter((f) => f.dir === "send").length;
          const recvN = frames.filter((f) => f.dir === "recv").length;
          console.log(
            `[${fmtTime(e.timestamp)}] ${e.method.padEnd(6)} ${stateName.padEnd(12)} —          ${e.url}`,
          );
        ...
        console.log(
          `[${fmtTime(e.timestamp)}] ${e.method.padEnd(6)} ${String(status).padEnd(4)} ${e.duration}ms  ${e.url}`,
        );
    ...
    case "errors": {
      ...
      for (const e of display) {
        ...
        if (e.mapped) {
          console.log(
            `  ↳ 原始源码: ${e.mapped.source}:${e.mapped.line}:${e.mapped.column}${e.mapped.name ? ` (${e.mapped.name})` : ""}`,
          );
        } else if (e.source) {
    ...
    case "inspect": {
      ...
      console.log(
        `- 类型: ${device?.deviceType ?? "unknown"} · ${device?.viewportWidth}×${device?.viewportHeight}`,
      );
    ...
    case "tags": {
      ...
      console.log(
        `✓ 已更新 [${id}] 标签：${(result.device.tags ?? []).join(", ") || "（无）"}${result.device.note ? "，备注：" + result.device.note : ""}`,
      );
      break;
    }
