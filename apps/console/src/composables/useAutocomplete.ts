...
const SILKPULSE_HELPERS = [
  "__silkpulse_click",
  "__silkpulse_setValue",
  "__silkpulse_type",
  "__silkpulse_pressKey",
  "__silkpulse_scroll",
  "__silkpulse_scrollIntoView",
  "__silkpulse_hover",
  "__silkpulse_wait",
  "__silkpulse_snapshot",
];
...
const STATIC_ITEMS: CompletionItem[] = [
  ...
  ...SILKPULSE_HELPERS.map((h) => ({
    text: h,
    label: h,
    kind: "helper" as const,
    detail: "silkpulse 辅助函数",
  })),
];
...
async function probeRemote(
  ...
  const code = mode === "root" ? buildRootProbeCode(prefix) : buildPropertyProbeCode(expr, prefix);
...
function getCompletions(
  ...
  if (ctx.mode === "root") {
    ...
    probeRemote(deviceId, "root", "", ctx.prefix).then((remoteKeys) => {
      ...
      const all = [
        ...remoteItems,
        ...staticMatched.filter((s) => !remoteItems.some((r) => r.text === s.text)),
      ];
