...
const diffPrepA = computed<DiffPrepared>(() => prepareDiffInput(diffA.value));
const diffPrepB = computed<DiffPrepared>(() => prepareDiffInput(diffB.value));

/** 测量解析耗时（在 computed 外手动计时，避免每次访问都重复计时） */
watch(
  [diffA, diffB, diffJsonMode, diffSortKeys],
  () => {
...
watch(
  [diffPrepA, diffPrepB, diffViewMode],
  async () => {
        ...
        <div class="flex-1 overflow-auto min-h-0">
          ...
          <div v-if="diffViewMode === 'object' && diffCanObjectView" class="flex gap-2 h-full">
            <div class="flex-1 min-w-0 flex flex-col border border-base rounded">
              <div
                class="px-2 py-1 text-xs text-muted bg-surface border-b border-base flex-shrink-0"
              >
                A（期望）
              </div>
              ...
              <template v-for="(item, i) in diffCollapsedInline.lines" :key="i">
                ...
                >
                  ...
                  <span
                    ...
                    >{{
                      item.line.type === "add" ? "+" : item.line.type === "del" ? "−" : " "
                    }}</span
                  >
                  <!-- 内容区 -->
                  <span
