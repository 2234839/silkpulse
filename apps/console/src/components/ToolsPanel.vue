...
const activeTool = ref<(typeof tools)[number]["id"]>(
  tools.some((t) => t.id === route.query.tool)
    ? (route.query.tool as (typeof tools)[number]["id"])
    : "json",
);
...
/** 解析模式：Auto（自动探测）/ JSON（标准）/ JSONC（带注释+尾逗号）/ JSON5（更宽松）/ JS（任意 JS 表达式） */
const jsonParseMode = ref<"auto" | "json" | "jsonc" | "json5" | "js">("auto");

/** 解析器函数表（auto 模式按此顺序逐个尝试） */
const parsers: Record<string, (text: string) => unknown> = {
  json: (t) => JSON.parse(t),
  jsonc: (t) => JSON.parse(stripTrailingCommas(stripComments(t))),
  json5: (t) => JSON.parse(json5ToStandardJson(t)),
  js: (t) => (0, eval)(`(${t})`),
};

/** 根据当前模式解析 JSON */
function parseByMode(text: string): unknown {
  if (jsonParseMode.value !== "auto") {
    return parsers[jsonParseMode.value](text);
  }
  /** auto：依次尝试 JSON → JSONC → JSON5 → JS，哪个能解析成功用哪个 */
  let lastErr: unknown;
  for (const id of ["json", "jsonc", "json5", "js"]) {
    try {
...
const jsonModeHint = computed(() => {
  if (jsonParseMode.value === "auto")
    return "Auto：自动探测格式（JSON → JSONC → JSON5 → JS 依次尝试）";
  if (jsonParseMode.value === "jsonc") return "JSONC：支持单行/多行注释、尾逗号";
  if (jsonParseMode.value === "json5")
    return "JSON5：支持注释、单引号、无引号 key、尾逗号、十六进制";
  if (jsonParseMode.value === "js")
    return "JS：任意 JS 表达式（对象/数组/函数/RegExp/Symbol/BigInt 等）";
...
function doEncode() {
  ...
  try {
    ...
    switch (codecMode.value) {
      case "Base64":
        codecResult.value = btoa(unescape(encodeURIComponent(s)));
        break;
      case "URL":
...
function doDecode() {
  ...
  try {
    ...
    switch (codecMode.value) {
      case "Base64":
        codecResult.value = decodeURIComponent(escape(atob(s)));
        break;
      case "URL":
...
const regexResult = computed(() => {
  if (!regexPattern.value || !regexInput.value || !regexReplace.value || regexError.value)
    return null;
  try {
    const flags = regexFlags.value.includes("g") ? regexFlags.value : regexFlags.value + "g";
...
<template>
  <div class="h-screen flex flex-col overflow-hidden bg-base">
    <!-- 顶部栏 -->
    <header
      class="bg-elevated border-b border-base px-4 py-2.5 flex items-center gap-4 flex-shrink-0"
    >
    ...
    <!-- 工具切换栏 -->
    <nav class="flex border-b border-base bg-surface overflow-x-auto no-scrollbar flex-shrink-0">
      <button
        v-for="t in tools"
        :key="t.id"
    ...
    <div class="flex-1 overflow-y-auto p-4">
      ...
      <div v-if="activeTool === 'json'" class="flex h-full max-h-[calc(100vh-160px)]">
        <div
          class="flex flex-col gap-2 min-w-0 flex-shrink-0"
          :style="{ width: toolLeftWidth + 'px' }"
        >
          <div class="flex items-center gap-2">
            ...
            <div class="ml-auto flex items-center gap-1">
              <button
                ...
                :key="m.id"
                @click="jsonParseMode = m.id as 'auto' | 'json' | 'jsonc' | 'json5' | 'js'"
          ...
          <div class="flex gap-2 items-center">
            <input
              ...
              type="text"
              spellcheck="false"
              :disabled="jsonParseMode === 'js' || jsonParseMode === 'auto'"
              ...
              :placeholder="
                jsonParseMode === 'js'
                  ? 'JS 模式下不可用（对象含函数/Symbol 无法序列化）'
                  : jsonParseMode === 'auto'
                    ? 'Auto 模式下不可用（可能解析为 JS 表达式）'
                    : 'JQ 过滤（如 .items[].msg）'
              "
            />
          </div>
        ...
        <div class="flex-1 overflow-auto min-w-0">
          ...
          <div
            v-else-if="jsonParsed && !jsonParsed.ok"
      ...
      <div v-else-if="activeTool === 'stream'" class="flex gap-4 h-full max-h-[calc(100vh-160px)]">
        ...
        <div
          class="flex-1 overflow-auto min-w-0 bg-surface border border-base rounded p-3 max-h-[calc(100vh-160px)]"
        >
          <div
            v-for="(e, i) in processedStreamEvents"
            :key="i"
            class="text-xs font-mono py-0.5 border-b border-base/30"
          >
      ...
      <div v-else-if="activeTool === 'regex'" class="flex gap-4 h-full max-h-[calc(100vh-160px)]">
        ...
        <div class="flex-1 overflow-auto min-w-0">
          ...
          >
            ...
            <pre class="text-xs text-green-500 whitespace-pre-wrap break-all">{{
              regexResult
            }}</pre>
          </div>
