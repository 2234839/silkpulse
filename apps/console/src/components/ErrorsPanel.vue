...
<template>
  <div class="flex-1 flex flex-col overflow-hidden bg-base">
    ...
    <div class="p-2 border-b border-base bg-surface flex items-center gap-2">
      ...
      >
        {{ copyingAllErrors ? "✓ 已复制" : `复制全部 (${filteredErrors.length})` }}
      </button>
    </div>
    <!-- 错误列表 -->
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
      ...
      >
        ...
        <div
          v-if="e.mapped"
          class="mt-1 text-xs text-blue-key bg-blue-soft border border-blue-soft rounded px-2 py-1 font-mono"
        >
      ...
      <div v-if="filteredErrors.length === 0" class="text-faint text-center py-8">
        {{ props.errors.length === 0 ? "暂无错误" : "无匹配错误" }}
      </div>
    </div>
  </div>
</template>
