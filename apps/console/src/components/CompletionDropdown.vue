...
const kindStyle = (kind: CompletionItem["kind"]): { color: string; badge: string } => {
  switch (kind) {
    case "keyword":
      return { color: "text-purple-500", badge: "K" };
    case "global":
      return { color: "text-blue-500", badge: "G" };
    case "dom":
      return { color: "text-orange-500", badge: "D" };
    case "helper":
      return { color: "text-green-500", badge: "H" };
    case "property":
      return { color: "text-cyan-600", badge: "P" };
    default:
      return { color: "text-faint", badge: "·" };
  }
};
...
<template>
  ...
  >
    <div
      ...
      :class="
        i === activeIndex ? 'bg-blue-soft text-blue-key' : 'text-primary hover:bg-blue-soft/50'
      "
    >
      <!-- kind badge -->
      <span
      ...
      <!-- 类型描述（如"内置全局"、"DOM id"） -->
      <span v-if="item.detail" class="ml-auto text-[10px] text-faint shrink-0">{{
        item.detail
      }}</span>
    </div>
  </div>
