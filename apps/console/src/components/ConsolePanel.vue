  ...
  if (completionVisible.value && completionItems.value.length) {
    ...
    if (e.key === "ArrowUp") {
      ...
      completionActive.value =
        completionActive.value <= 0 ? completionItems.value.length - 1 : completionActive.value - 1;
      return;
    }
    if (e.key === "ArrowDown") {
      e.preventDefault();
      completionActive.value =
        completionActive.value >= completionItems.value.length - 1 ? 0 : completionActive.value + 1;
      return;
    }
    if (e.key === "Escape") {
      e.preventDefault();
...
<template>
  <div class="flex-1 flex flex-col overflow-hidden bg-base">
    ...
    <div class="flex items-center gap-2 px-4 py-2 border-b border-base bg-surface">
      <div class="flex gap-1">
        ...
        >
          ...
          <span v-if="lvl !== 'all' && levelCount(lvl) > 0" class="ml-1 opacity-75">{{
            levelCount(lvl)
          }}</span>
        </button>
      </div>
    ...
    <div ref="logListEl" class="flex-1 overflow-y-auto p-4 font-mono text-sm">
      ...
      >
        <span class="text-faint text-xs mr-2">{{
          new Date(log.timestamp).toLocaleTimeString()
        }}</span>
        <span class="text-faint text-xs mr-2 uppercase">{{ log.type }}</span>
        <!-- 有 styledSegments 时渲染带样式的文本段（console.log %c），否则降级纯文本 -->
        <span
          v-if="log.styledSegments?.length"
          :class="logColor(log.type)"
          class="break-all whitespace-pre-wrap"
        >
          <span v-for="(seg, si) in log.styledSegments" :key="si" :style="seg.style">{{
            seg.text
          }}</span>
        </span>
        <span v-else :class="logColor(log.type)" class="break-all whitespace-pre-wrap">{{
          log.message
        }}</span>
        <span v-if="log.repeat" class="text-[10px] text-faint ml-1 shrink-0"
          >×{{ log.repeat }}</span
        >
        <span class="text-[10px] text-blue-key opacity-0 group-hover:opacity-60 ml-1">{{
          copyingLogTs === log.timestamp ? "✓" : "复制"
        }}</span>
      </div>
      <div
        v-if="displayLogs.length === 0 && execResults.length === 0"
      ...
      <div v-if="execResults.length" class="mt-2 border-t border-light pt-2">
        ...
        >
          ...
          <div class="flex items-baseline gap-2 mt-0.5">
            <span class="text-faint text-xs shrink-0 opacity-0">{{
              new Date(r.time).toLocaleTimeString()
            }}</span>
            <span :class="r.ok ? 'text-green-600' : 'text-red-500'" class="shrink-0">{{
              r.ok ? "←" : "✗"
            }}</span>
            <!-- 有结构化值时用对象树，否则降级纯文本 -->
    ...
    <div
      class="border-t border-base bg-surface px-3 py-2 flex items-center gap-2 shrink-0 relative"
    >
      <!-- 补全下拉列表（悬浮在输入行上方） -->
      <CompletionDropdown
