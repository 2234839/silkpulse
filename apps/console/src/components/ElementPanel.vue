...
/**
 * 统一 toggle 入口：普通 children + shadow 子树作为整体展开/收起
 *
 * 展开时两者并行加载、同时展开（一次点击到位）；收起时同时收起。
 * 旧实现拆成两段（先 children 后 shadow），导致 shadow host 节点
 * 需要点三次才能收起，箭头一直是展开态看起来“点击没反应”。
 */
async function handleToggle(node: ElementNode) {
  if (!hasExpandable(node)) return;
  ...
  /** 整体展开：普通 children + shadow 子树并行加载 */
  node.loading = true;
  try {
    const tasks: Promise<void>[] = [];
    if (node.childCount > 0 && !node.children) {
      tasks.push(
        loadElementChildren(node).then((children) => {
          node.children = children;
        }),
      );
    }
    if (node.hasShadow && !node.shadowChildren) {
      tasks.push(
        loadElementChildren(node, true).then((shadowChildren) => {
    ...
    if (node.hasShadow) node.shadowExpanded = true;
  } finally {
    node.loading = false;
  }
}

/** 节点整体是否展开（普通或 shadow 任一展开） */
function isNodeExpanded(n: ElementNode): boolean {
  return !!n.expanded || !!n.shadowExpanded;
}

/** 节点是否有可展开内容（与 ElementTreeNode.hasExpandable 同语义） */
...
async function refreshChangedNodes(changes: DomChangeData) {
  ...
  for (const parentIdx of changes.parentIdxs) {
    ...
    /**
     * 未展开的节点：刷新元信息（childCount 可能变了）。
     *
     * 旧实现只刷新已展开节点，收起状态下的 “…N” 子元素计数永远不更新，
     * DOM 变了用户看不到，展开时还会拿到旧计数。
     */
    if (!node.expanded) {
      try {
        const fresh = (await loadElementChildren(node, false)).length;
        if (node.childCount !== fresh) {
          node.childCount = fresh;
          rebuildIndex(elementTreeRoot.value);
        }
      } catch {
        /** 静默 */
