...
const props = defineProps<{
  ...
  onGuestCreateProject?: (name?: string) => Promise<{ apiKey: string; projectId: string; projectName: string; expiresAt?: string } | null>
}>()
...
/** 创建结果（一次性展示） */
const guestResult = ref<{ apiKey: string; projectId: string; projectName: string; expiresAt?: string } | null>(null)
/** 复制状态 */
...
<template>
  <div class="min-h-screen flex flex-col lg:flex-row bg-gray-900 text-white">
    <!-- 左侧：产品自述 -->
    <div class="flex-1 flex flex-col justify-center px-6 py-10 sm:px-10 lg:px-16 lg:py-0 lg:max-w-[55%]">
      <div class="max-w-xl mx-auto lg:mx-0">
        <!-- Logo + 品牌名 -->
        <div class="flex items-center gap-3 mb-8">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-xl shadow-lg shadow-blue-500/30">
            🩺
          </div>
          <span class="text-2xl font-bold tracking-tight">SilkPulse</span>
          <span class="px-2 py-0.5 text-[10px] font-medium rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/20">悬丝诊脉</span>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
          <div class="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
            ...
            <p class="text-gray-500 text-xs leading-relaxed">console / 网络 / 错误 / DOM 快照，穿透 shadow DOM + iframe</p>
          </div>
          <div class="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
            <div class="text-lg mb-1.5">🤖</div>
            <h3 class="font-semibold text-sm mb-1">AI 操作</h3>
            <p class="text-gray-500 text-xs leading-relaxed">AI 直接在远程页面点击、输入、滚动、截图，像人类一样操作</p>
          </div>
          <div class="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
            <div class="text-lg mb-1.5">📡</div>
            <h3 class="font-semibold text-sm mb-1">多设备并发</h3>
            <p class="text-gray-500 text-xs leading-relaxed">同时调试多台设备，断线重连不丢历史，环形缓冲区兜底</p>
          </div>
          <div class="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
            <div class="text-lg mb-1.5">🧩</div>
            <h3 class="font-semibold text-sm mb-1">零侵入接入</h3>
            <p class="text-gray-500 text-xs leading-relaxed">script 标签 / bookmarklet / userscript，不改源码也能接入</p>
          </div>
        </div>
    ...
    <!-- 右侧：登录区域 -->
    <div class="flex-1 flex items-center justify-center px-6 py-10 sm:px-10 lg:py-0 lg:max-w-[45%] bg-gray-950/50 border-t lg:border-t-0 lg:border-l border-gray-800">
      <div class="w-full max-w-sm space-y-6">
          ...
          <div v-if="guestProjectsEnabled" class="pt-4">
            ...
            <div v-if="!guestResult" class="space-y-2">
              ...
              <div class="flex gap-2">
                ...
                >{{ authLoading ? '创建中...' : '🔑 创建我的 Key' }}</button>
              </div>
            </div>
            ...
            <div v-else class="space-y-3">
              <div class="bg-yellow-900/20 border border-yellow-700/40 rounded-lg p-3 space-y-2">
                ...
                <code class="block px-2 py-1.5 text-xs bg-gray-900 rounded font-mono break-all text-green-400 select-all">{{ guestResult.apiKey }}</code>
                <div class="flex gap-2">
                  <button
              ...
              <p class="text-[11px] text-gray-500">
                项目「{{ guestResult.projectName }}」· {{ fmtExpires(guestResult.expiresAt) }}自动销毁 ·
                Project ID: <code class="text-gray-400">{{ guestResult.projectId }}</code>
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- 游客模式提示 -->
        <div v-if="playgroundEnabled || guestProjectsEnabled" class="bg-yellow-900/20 border border-yellow-700/30 rounded-lg p-3">
          <p class="text-xs text-yellow-400/80 leading-relaxed">
            🎮 以上是对游客的限制：数据在公网传输、项目最长存活 5 天、到期自动销毁<br/>
            <a :href="GITHUB_URL" target="_blank" rel="noopener noreferrer" class="text-yellow-600 hover:text-yellow-500 underline underline-offset-2">有长期需要建议自己部署一份（开源免费，顺手求个 ⭐）</a>
          </p>
        </div>
      </div>
    </div>
