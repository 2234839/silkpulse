...
const {
  history: execHistory,
  record: recordExec,
  remove: removeExecHistory,
  clear: clearExecHistory,
} = useExecHistory();
...
<template>
  <div class="flex-1 flex overflow-hidden bg-base">
      ...
      <div class="flex-1 overflow-y-auto p-3">
        <template v-if="execResult">
          ...
          <div v-if="execLogs.length" class="mt-2">
            ...
            <pre class="text-xs font-mono text-secondary whitespace-pre-wrap">{{
              execLogs.join("\n")
            }}</pre>
          </div>
          <!-- 执行后快照：默认折叠 -->
          <details v-if="execSnapshot" class="mt-3">
            <summary class="text-xs text-blue-600 cursor-pointer select-none hover:text-blue-700">
              ▶ 执行后快照（{{ execSnapshot.length }} 字符，点击展开）
            </summary>
            <pre
              class="text-xs font-mono text-secondary whitespace-pre-wrap mt-2 p-2 bg-surface rounded border border-base"
              >{{ execSnapshot }}</pre>
          </details>
        </template>
    ...
    <!-- 历史侧栏 -->
    <div
      class="border-l border-base bg-surface flex flex-col overflow-hidden flex-shrink-0"
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        >
          <div class="flex items-center justify-between mb-0.5">
            <span class="text-[10px] font-mono" :class="h.ok ? 'text-green-600' : 'text-red-500'">{{
              h.ok ? "✓" : "✗"
            }}</span>
            <button
