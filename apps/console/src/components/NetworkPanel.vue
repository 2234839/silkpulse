    ...
    >
      ...
      <div class="p-2 border-b border-light bg-surface space-y-2">
        <span
          ...
          title="长会话缓冲区已满，最早的记录已被滚动淘汰（新记录不受影响）"
      ...
      <div class="flex-1 overflow-y-auto">
        <table class="w-full text-sm table-fixed">
          <thead class="bg-elevated text-secondary text-xs uppercase sticky top-0">
            <tr>
              <th class="w-[74px] whitespace-nowrap text-left px-2 py-2">时间</th>
              <th class="w-[46px] whitespace-nowrap text-left px-2 py-2">方法</th>
              <th class="w-[46px] whitespace-nowrap text-left px-2 py-2">状态</th>
              <th class="whitespace-nowrap text-left px-2 py-2">URL</th>
              <th class="w-[62px] whitespace-nowrap text-right px-2 py-2">大小</th>
              <th class="w-[56px] whitespace-nowrap text-right px-2 py-2">
                <button
          ...
          <tbody>
            ...
            >
              <td
                class="px-2 py-2 text-faint text-xs font-mono overflow-hidden text-ellipsis whitespace-nowrap"
              >
                {{ new Date(n.timestamp).toLocaleTimeString() }}
              </td>
              <td class="px-2 py-2 text-secondary font-mono text-xs">{{ n.method }}</td>
              <td
              ...
              <td class="px-2 py-2 text-primary truncate text-xs">
                <span
                  v-if="n.sseState"
              ...
              <td class="px-2 py-2 text-right text-xs font-mono text-muted whitespace-nowrap">
                {{ n.protocol === "ws" ? "—" : formatSize(calcResSize(n)) }}
              </td>
              <td
                class="px-2 py-2 text-right text-xs font-mono"
                :class="
