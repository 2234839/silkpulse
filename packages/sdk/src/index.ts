...
function tryHoldTabLock(deviceId: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    locks
      .request(`silkpulse-tab-${deviceId}`, { ifAvailable: true }, async (lock) => {
        if (!lock) {
          resolve(false);
          return;
        }
...
function detectFrameworks(): string[] {
  ...
  const reactHook = (
    window as unknown as {
      __REACT_DEVTOOLS_GLOBAL_HOOK__?: { renderers?: Map<string, unknown> | unknown[] };
    }
  ).__REACT_DEVTOOLS_GLOBAL_HOOK__;
  let reactRegistered = false;
  if (reactHook?.renderers) {
    const count =
      reactHook.renderers instanceof Map
        ? reactHook.renderers.size
        : (reactHook.renderers as unknown[]).length;
...
async function initWithDeviceId(options: InitOptions): Promise<void> {
  ...
  installStorageWatcher((storageType, key, timestamp) =>
    send({ type: "storage-change", storageType, key, timestamp }),
  );
