...
interface ReactRendererInterface {
  ...
  overrideHookState?:
    | ((fiber: object, hookID: number, path: string[], value: unknown) => void)
    | null;
...
function getReactRenderer(): {
  ...
  if (!interfaces || interfaces.size === 0)
    return { error: "页面没有 React 应用（或 react-dom 未注册 renderer）" };
...
export function installDevToolsHelpers(): void {
  ...
  w.__silkpulse_devtools_available = (): { react: boolean; vue: boolean } => {
    const reactHook = window as unknown as {
    ...
    if (!vue) {
      for (const el of document.querySelectorAll("#app, #root, body > div")) {
        if ((el as unknown as { __vue_app__?: unknown }).__vue_app__) {
          vue = true;
          break;
        }
      }
   ...
   */
  w.__silkpulse_devtools_tree = async (): Promise<{
    framework?: string;
    tree?: DevToolsComponentNode[];
    error?: string;
  }> => {
  ...
  async function buildReactTree(): Promise<{ framework: string; tree: DevToolsComponentNode[] }> {
    ...
    if (!hook)
      return err("React backend 激活失败") as unknown as {
        framework: string;
        tree: DevToolsComponentNode[];
      };
    ...
    /** 0=FunctionComponent 1=ClassComponent 11=ForwardRef 10=MemoComponent 15=SimpleMemoComponent */
    const isComponentTag = (tag: unknown): boolean =>
      tag === 0 || tag === 1 || tag === 11 || tag === 10 || tag === 15;
    ...
    const collectChildren = (
      ...
      const walk = (first: Record<string, unknown> | null, d: number): void => {
        ...
        for (
          ...
          cur;
          cur = cur.sibling as Record<string, unknown> | null
        ) {
    ...
    for (const root of roots) {
      const rootFiber =
        (root as { current?: Record<string, unknown> }).current ??
  ...
  function domBasedReactRoots(): object[] {
    ...
    for (const el of document.querySelectorAll("#root, #app, [data-reactroot], body > div")) {
      const key = Object.getOwnPropertyNames(el).find(
  ...
  async function inspectReact(el: Element): Promise<Record<string, unknown>> {
    ...
    const hostEl =
      compFiber.stateNode instanceof Element ? compFiber.stateNode : firstHostElement(compFiber);
    ...
    const data =
      inspected.type === "full-data" && inspected.value != null
  ...
  function firstHostElement(fiber: Record<string, unknown>): Element | null {
    ...
    for (
      let c = fiber.child as Record<string, unknown> | null;
      c;
  ...
  async function setReact(
    ...
    if (type === "hooks") {
      if (!Number.isInteger(hookID))
        return err("hooks 写入需要 hookID 参数（inspect 结果 hooks 数组的下标）");
      if (typeof renderer!.overrideHookState !== "function") {
        return err(
          "目标页 React 是生产构建（bundleType=0），react-dom 未提供 overrideHookState（仅 dev 构建可用），无法写 hooks",
        );
      }
      ...
      if (type === "props") {
        if (typeof renderer!.overrideProps !== "function")
          return err("目标页 React 是生产构建，无法写 props");
  ...
  function vueRootInstances(): VueInstance[] {
    ...
    for (const app of hook?.apps ?? []) {
      const rec = (app as { __VUE_DEVTOOLS_NEXT_APP_RECORD__?: { rootInstance?: VueInstance } })
  ...
  function getVueComponentFromDOM(el: Element): { instance?: VueInstance; error?: string } {
    ...
    const visit = (ins: VueInstance, d: number): void => {
      ...
      if (
        host === el ||
        (host && el.compareDocumentPosition(host) & Node.DOCUMENT_POSITION_CONTAINS)
      ) {
      ...
      const collect = (vnode: Record<string, unknown> | null | undefined, cd: number): void => {
        ...
        if (vnode.component) {
          visit(vnode.component as VueInstance, d + 1);
          return;
        }
        const kids = vnode.children;
  ...
  function vueEditableState(instance: VueInstance): Record<string, unknown> | undefined {
    return instance.setupState && Object.keys(instance.setupState).length > 0
      ? instance.setupState
      : instance.data;
  }
  ...
  function setVue(
    ...
    if (type === "hooks")
      return err("Vue 没有 hooks，用 state（setup 返回的 ref/reactive 都在 state 里）");
    const { instance, error } = getVueComponentFromDOM(el);
    if (error) return err(error);
    const ins = instance!;
    const container: Record<string, unknown> | undefined =
      type === "props" ? ins.props : vueEditableState(ins);
    ...
    for (const seg of path.slice(0, -1)) {
      ...
      if (!next || typeof next !== "object")
        return err(`路径 ${JSON.stringify(path)} 在 ${String(seg)} 处中断（值非对象）`);
      cur = next as Record<string, unknown>;
    }
    const last = path.length > 0 ? path[path.length - 1] : "";
    if (path.length > 0 && !(last in cur)) {
      return err(
        `属性 ${last} 不存在（响应式对象不能凭空新增根级属性，请核对 inspect 输出的字段名）`,
      );
    }
