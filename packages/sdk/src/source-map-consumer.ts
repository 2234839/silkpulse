  ...
  for (const row of rows) {
    ...
    if (row.length > 0) {
      ...
      for (const field of fields) {
        ...
        let pos = 0
        const [genColDelta, p1] = decodeVlq(field, pos); pos = p1
        ...
        if (pos < field.length) {
          const [srcDelta, p2] = decodeVlq(field, pos); pos = p2
          const [lineDelta, p3] = decodeVlq(field, pos); pos = p3
          const [colDelta, p4] = decodeVlq(field, pos); pos = p4
          prevSource += srcDelta
          ...
          if (pos < field.length) {
            const [nameDelta, p5] = decodeVlq(field, pos); pos = p5
            prevName += nameDelta
            seg.name = prevName
          }
        }
