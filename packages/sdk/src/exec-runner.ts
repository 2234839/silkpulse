...
import { ignoreDomSubtree } from "./dom-watcher.js";
  ...
  try {
    return (
      JSON.stringify(
        ...
        0,
      )?.slice(0, MAX_RESULT_LEN) ?? "undefined"
    );
  } catch (e) {
...
export function installHelpers(): void {
  ...
  w.__silkpulse_setValue = (idx: number, val: string): boolean => {
    ...
    if (el.tagName === "INPUT" && (el.type === "checkbox" || el.type === "radio")) {
      ...
      const shouldCheck =
        el.type === "radio" ? true : val === "true" || val === "1" || val === "checked";
      /**
  ...
  w.__silkpulse_hover = (idx: number): boolean => {
    ...
    if (prev && prev !== target && prev.isConnected) {
      prev.dispatchEvent(
        new MouseEvent("mouseout", { bubbles: true, cancelable: true, relatedTarget: target }),
      );
      prev.dispatchEvent(
        new MouseEvent("mouseleave", { bubbles: false, cancelable: true, relatedTarget: target }),
      );
    }
  ...
  w.__silkpulse_storage = (
    ...
    const truncate = (s: string): string =>
      s.length > MAX_VAL ? s.slice(0, MAX_VAL) + `…(${s.length})` : s;
    if (type === "cookie") {
  ...
  w.__silkpulse_screenshot = async (
    ...
    /**
     * DOM 噪声隔离：snapdom 会向 document 挂临时容器（克隆 DOM 渲染），
     * 这些操作若被 dom-watcher 当页面变化上报，控制台 DOM 树会被无关刷新
     * （破坏展开状态）。
     *
     * 方案：挂一次性 body observer 捕获截图期间新增的顶层节点，
     * 标记为 ignored subtree（dom-watcher 只跳过这些容器关联的变化，
     * 并发的真实页面变化照常上报，不做时间维度的整体静默）。
     * 截图结束后 disconnect。
     */
    const noiseObserver = new MutationObserver((muts) => {
      for (const m of muts) {
        for (const n of m.addedNodes) {
          if (n instanceof Element) ignoreDomSubtree(n);
        }
      }
    });
    noiseObserver.observe(document.documentElement, { childList: true, subtree: false });
    /** body 挂载点也要观察（snapdom 容器多挂在 body 下） */
    noiseObserver.observe(document.body, { childList: true, subtree: false });
    ...
    /** 根据格式选择 SnapDOM 导出方法（finally 里 disconnect 噪声 observer） */
    try {
      if (format === "png") {
      ...
      if (format === "webp") {
        const img = await snapdom.toWebp(target, {
          scale,
          backgroundColor,
          quality,
          fast: true,
      ...
      /** 默认 jpg */
      const img = await snapdom.toJpg(target, {
        scale,
        backgroundColor,
        quality,
        fast: true,
