...
export function installLogCollector(sink: LogSink, repeatSink: RepeatSink): void {
  ...
  console.table = (data?: unknown, columns?: string[]) => {
    ...
    if (Array.isArray(data)) {
      const rows = data
        .slice(0, 20)
        .map((row, i) => `  [${i}]: ${stringify(row, 1, new WeakSet())}`);
  ...
  while ((match = formatRe.exec(first)) !== null) {
    ...
    if (spec === "%c") {
      ...
      switch (spec) {
        ...
        case "%d":
        case "%i":
          currentText += String(parseInt(String(val), 10));
          break;
        case "%f":
          currentText += String(parseFloat(String(val)));
          break;
        case "%o":
        case "%O":
          currentText += stringify(val, 0, seen);
          break;
      }
    }
  }
  ...
  try {
    ...
    if (val instanceof Map) {
      ...
      const pairs = entries.map(
        ([k, v]) =>
          `${stringify(k, depth + 1, seen, maxDepth)} => ${stringify(v, depth + 1, seen, maxDepth)}`,
      );
    ...
    if (val instanceof Set) {
      ...
      const items = [...val.values()]
        .slice(0, 10)
        .map((v) => stringify(v, depth + 1, seen, maxDepth));
      const ellipsis = val.size > 10 ? ", ..." : "";
      seen.delete(val);
