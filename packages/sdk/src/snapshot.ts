...
/** 不可见的子元素标签 */
const INVISIBLE_TAGS: Record<string, true> = {
  SCRIPT: true,
  STYLE: true,
  OPTION: true,
  OPTGROUP: true,
...
function captureVisualStyle(el: HTMLElement): SnapshotElement["style"] | null {
  ...
  if (padTop > 0 || padLeft > 0) {
    /** 取四边最大值（简化），按缩放渲染时再调整 */
    s.pad = Math.round(
      Math.max(padTop, padLeft, parseFloat(cs.paddingBottom), parseFloat(cs.paddingRight)),
    );
  }
...
function isLeaf(el: Element): boolean {
  ...
  for (const child of el.children) {
    if (!(child.tagName in INVISIBLE_TAGS) && (child as HTMLElement).offsetParent !== null)
      return false;
  }
  return true;
}
...
function processElement(
  ...
  const hasValue =
    (tagLower === "input" || tagLower === "textarea" || tagLower === "select") && !!htmlEl.value;
