...
export async function createServer(
  options: SilkPulseServerOptions = {},
): Promise<uWS.TemplatedApp> {
  const port = options.port ?? 8080;

  /** 项目数据持久化路径：优先 options，否则用环境变量，最后默认 ~/.silkpulse/projects.json */
  const defaultDataDir =
    process.env.SILKPULSE_DATA_DIR ?? path.join(process.env.HOME ?? "/tmp", ".silkpulse");
  ...
  const wsUpgrade =
    (wsPath: string) =>
    (res: uWS.HttpResponse, req: uWS.HttpRequest, context: uWS.us_socket_context_t) => {
  ...
  app.any("/*", async (res, req) => {
    ...
    if (
      ctx.url.startsWith("/api/projects") ||
      ctx.url.startsWith("/api/auth") ||
      ctx.url.startsWith("/api/guest")
    ) {
      await readBody(ctx);
    ...
    if (pathname === "/sdk.js") {
      ...
      if (fs.existsSync(sdkPath)) {
        ...
        writeResponse(
          ...
          "// sdk.js 未构建，请在 packages/sdk 执行构建",
        );
      }
      return;
    }
    ...
    if (pathname === "/favicon.ico") {
      ...
      const faviconPng = Buffer.from(
        "iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAIAAAD8GO2jAAAAXUlEQVR4nGOwbvpGU8QwasGoBTD0HwNQzQJMo0myBp8F+I0m0hqcFhBvOn47BsgCUk3HY8dAWECe6bjsGLVgZFow9PMBPYoKelhA8+KaSGsIah8EVSaFaNSCEWABAEdJY9GqZCFVAAAAAElFTkSuQmCC",
        "base64",
      );
      writeResponse(
        ctx,
    ...
    if (pathname === "/demo" || pathname === "/demo.html") {
      ...
      if (demoPagePath && fs.existsSync(demoPagePath)) {
        ...
        let html = fs
          .readFileSync(demoPagePath, "utf8")
          .replace(/https?:\/\/localhost:8080/g, demoOrigin);
    ...
    if (
      pathname === "/inject/iife" ||
      pathname === "/inject/bookmarklet" ||
      pathname === "/inject/userscript"
    ) {
      /** 根据请求 Host header 推断 origin，本地用 localhost:port，线上用实际域名 */
      ...
      if (pathname === "/inject/iife") {
        writeResponse(
          ...
          injectScriptCode(origin, projectId),
        );
        ...
        writeResponse(
          ...
          200,
          { "Content-Type": "text/plain; charset=utf-8" },
          buildBookmarklet(origin, projectId),
        );
      } else {
        writeResponse(
          ctx,
          200,
          { "Content-Type": "text/javascript; charset=utf-8" },
          buildUserscript(origin, projectId),
        );
      }
      return;
    }
    ...
    if (pathname === "/inject-test") {
      writeResponse(
        ...
        200,
        { "Content-Type": "text/html; charset=utf-8" },
        `<!DOCTYPE html><html><head><meta charset="utf-8"><title>inject 测试页</title></head><body><h1>空白测试页</h1><p>用于验证 bookmarklet/userscript 注入</p></body></html>`,
      );
      return;
    }
    ...
    if (pathname === "/test-fixtures/crash.js") {
      writeResponse(
        ...
        'function n(r){throw new Error(r)}n("source map 测试错误")\n//# sourceMappingURL=crash.js.map',
      );
      return;
    }
  ...
  await new Promise<void>((resolve, reject) => {
    app.listen(port, (listenSocket) => {
      if (listenSocket) {
        ...
        console.log(
          `  接入设备：在目标页面注入 <script src="http://localhost:${port}/sdk.js"></script>`,
        );
        console.log(`  AI 接入：HTTP API → http://localhost:${port}/api/devices`);
        if (auth.isAuthEnabled()) {
          console.log(
            `  🔒 鉴权已启用（${auth.hasAdminKey() ? "超管密钥 + " : ""}${projectStore.list().length} 个项目）`,
          );
        } else {
...
function serveFile(
  ...
  try {
    ...
    const { body, headers: gzipHeaders } = maybeGzipResponse(
      { headers: ctx.headers },
      fileBuf,
      headers,
    );
