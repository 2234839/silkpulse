...
import {
  ...
  sendText,
  readBody,
  buildElementTreeCode,
  buildElementFilterCode,
} from "./api.js";
...
export async function handleAgentApiRoute(
  ...
  if (!device) {
    sendText(
      ctx,
      `[错误] 设备 ${deviceId} 不在线。先 GET /api/agent/devices 查看在线设备列表。`,
      404,
    );
    return true;
  }
  ...
  switch (action) {
    ...
    case "inspect": {
      ...
      const failedNetwork = device.network
        .all()
        .filter((n) => n.status === 0 || n.status >= 400)
        .slice(-limitParam(10));
      const snapshotResult = await execOnDevice(
        registry,
        deviceId,
        "return __silkpulse_snapshot()",
      );
      const parts: string[] = [];
      ...
      if (errors.length > 0) {
        ...
        for (const e of errors) {
          const source = e.mapped
            ? ` → ${e.mapped.source}:${e.mapped.line}`
            : e.source
              ? ` (${e.source}:${e.line})`
              : "";
