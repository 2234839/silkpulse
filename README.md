...
# 一键诊断聚合（错误 + 失败网络 + 慢请求 + 日志 + 快照，AI 最高效入口）
...
└── scripts/
    └── headless-test.mjs # 无头浏览器端到端测试（89 项）
```
