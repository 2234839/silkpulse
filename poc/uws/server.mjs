...
const compressor =
  COMPRESS_MODE === "shared"
    ? uWS.SHARED_COMPRESSOR
    : COMPRESS_MODE === "dedicated"
      ? uWS.DEDICATED_COMPRESSOR(3 * 1024)
      : 0;
...
app.ws("/ws/console", {
  ...
  open: (ws) => {
    ...
    ws.send(
      JSON.stringify({ type: "device-list", devices: [...devices.values()].map((d) => d.info) }),
    );
  },
  message: (ws, data) => {
    let msg;
    ...
    if (msg.type === "subscribe") {
      ...
      if (set) {
        set.delete(ws);
        if (!set.size) watchers.delete(msg.deviceId);
      }
