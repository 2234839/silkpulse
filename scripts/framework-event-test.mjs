...
async function main() {
    ...
    for (const test of group.tests) {
      ...
      try {
        ...
        if (isResultBased) {
          ...
          if (resultValue.includes('too-fast') || resultValue.includes('no-scroll') || resultValue.includes('not-visible') || resultValue.includes('empty')) {
            ok = false
          }
        } else {
  ...
  console.log(`  ${PASS} 通过: ${passed}   ${FAIL} 失败: ${failed}   ${SKIP} 跳过: ${skipped}   总计: ${total}`)
  console.log('═'.repeat(72))
  console.log('')
