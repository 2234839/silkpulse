...
async function execOn(deviceId, code) {
  ...
  if (!data.success) {
    ...
    console.error(
      `    [当前在线] ${JSON.stringify((devs.devices ?? []).map((d) => ({ id: d.id, title: d.title })))}`,
    );
...
async function waitDevice(urlHint, timeoutMs = 20000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (dev) {
      ...
      try {
        const probe = await api(`/api/devices/${dev.id}/exec`, {
          method: "POST",
...
async function launchCase() {
  ...
  return puppeteer.launch({
    ...
    args: [
      ...
      `--user-data-dir=${path.join(profileDir, `matrix-${caseSeq}`)}`,
    ],
  });
}
...
async function runCase(cfg) {
  ...
  try {
    ...
    const idx = await execOn(
      dev.id,
      `__silkpulse_ensureIdx(document.querySelector('${cfg.idxSel}'))`,
    );
    ...
    const setResult =
      cfg.framework === "vue"
        ? await execOn(
            dev.id,
            `__silkpulse_devtools_set(${idx}, 'state', ${JSON.stringify(cfg.setPath)}, ${JSON.stringify(cfg.setVal)})`,
          )
        : await execOn(
            dev.id,
            `__silkpulse_devtools_set(${idx}, 'hooks', [], ${JSON.stringify(cfg.setVal)}, ${cfg.hookID})`,
          );
    await sleep(300);
    const counterText = await execOn(
      dev.id,
      `document.querySelector('${cfg.counterSel}')?.textContent?.trim()`,
    );
    if (setResult?.success === true && counterText === String(cfg.setVal)) {
      ...
      fail(
        "set 后 UI 未更新",
        `result=${JSON.stringify(setResult).slice(0, 200)} counter=${counterText}`,
      );
    }
    ...
    clickExec === "clicked" && afterClick === expectAfter
      ...
      : fail(
          "恢复后点击异常",
          `clickExec=${clickExec} counter=${afterClick}（期望 ${expectAfter}）`,
        );
  } finally {
    await browser.close();
  }
