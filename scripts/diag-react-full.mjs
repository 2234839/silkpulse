...
const exec = async (code) => {
  const r = await (
    await api(`/api/devices/${dev.id}/exec`, { method: "POST", body: JSON.stringify({ code }) })
  ).json();
...
const clickOk = await p.click("#inc-btn").then(
  () => true,
  async () => {
    try {
      await p.evaluate(() => document.querySelector("#inc-btn")?.click());
      return true;
    } catch {
      return false;
    }
