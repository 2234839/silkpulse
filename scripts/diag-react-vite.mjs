...
async function runCase(name, url, preInject) {
  ...
  const b = await puppeteer.launch({
    ...
    args: [
      ...
      `--user-data-dir=${path.join(os.homedir(), `snap/chromium/common/tmp-profiles/rv-${Date.now()}`)}`,
    ],
  });
  ...
  try {
    ...
    if (!dev.frameworks?.includes("react")) {
      fail("frameworks 未含 react");
      return { pass, fail: failCount };
    }
    ...
    avail?.react === true
      ? ok("available: react=true")
      : fail("available 失败", JSON.stringify(avail ?? null));
    ...
    const idx = await execOn(
      dev.id,
      `return __silkpulse_ensureIdx(document.querySelector('#inc-btn'))`,
    );
    ...
    const setRes = await execOn(
      dev.id,
      `return __silkpulse_devtools_set(${idx}, 'hooks', [], 100, 0)`,
    );
    setRes?.error
      ? ok(`set 如实报告受限（prod 预期）：${String(setRes.error).slice(0, 50)}`)
      : setRes?.success
        ? ok("set 成功（意外但可接受）")
        : fail("set 返回异常", JSON.stringify(setRes ?? null).slice(0, 200));

    const before = await execOn(
      dev.id,
      `return document.querySelector('.counter')?.textContent?.trim()`,
    );
    const click = await execOn(
      dev.id,
      `return (() => {
...
const r1 = await runCase(
  "真实 React vite build + 先注入（ESM chunk 异步 + stub 提前就位）",
  `${SERVER}/react-vite-post.html`,
  true,
);
const r2 = await runCase(
  "真实 React vite build + 后注入（无全局 React，dispatcherRef 探测受限）",
  `${SERVER}/react-vite-late.html`,
  false,
);
