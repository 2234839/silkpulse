...
async function assertFullWidth(page, name) {
  ...
  console.log(
    `  [${name}] viewport=${geo.viewport} main=${geo.mainRight} nav=${geo.navRight} ${ok ? "✓ 满宽" : "✗ 未满宽"}`,
  );
  return ok;
}
...
async function main() {
  ...
  try {
    ...
    await demo.evaluate(() => {
      ...
      console.log(
        "[order#98123] 支付成功 · 3 items · ¥1,298.00 · wxpay · polls=3 · 842ms · gateway=cache-hit",
      );
    ...
    await consolePage.evaluate(() => {
      const item = [...document.querySelectorAll("aside li")].find((li) =>
        li.textContent?.includes("demo"),
      );
      item?.click();
    });
    await sleep(2500);
    ...
    await consolePage.evaluate(() => {
      ...
      if (ta) {
        ta.value =
          "return { title: document.title, url: location.href, viewport: innerWidth + 'x' + innerHeight }";
