...
class WsAdapter {
  constructor(url) {
    ...
    this.raw.addEventListener("error", (ev) =>
      dispatch("error", new Error(ev.message ?? "ws error")),
    );
  }
  on(type, fn) {
    ...
    if (r.ok) {
      ...
      fanout =
        j.fanoutSent != null
          ? {
              sent: j.fanoutSent,
              skippedClosed: j.fanoutSkippedClosed,
              skippedProject: j.fanoutSkippedProject,
              backpressureClosed: j.fanoutBackpressureClosed,
            }
          : null;
    }
  } catch {}
...
function waitOnline(id, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      if (i >= 0) {
        cur.splice(i, 1);
        if (!cur.length) observer.seen.delete(id);
      }
...
async function rampDevices(n, batchSize = 25) {
  ...
  for (let b = 0; b < n; b += batchSize) {
    ...
    const batch = await Promise.all(Array.from({ length: size }, (_, j) => spawnDevice(b + j)));
    devices.push(...batch);
    times.push(...batch.map((d) => d.tRegister));
    process.stdout.write(
      `  接入 ${devices.length}/${n}（批次耗时 ${fmtMs(batch[batch.length - 1].tRegister)}）\r`,
    );
  }
  console.log(`  ✓ ${n} 设备在线，总耗时 ${fmtMs(performance.now() - t0)}`);
  console.log(
    `  register 延迟: p50=${fmtMs(pct(times, 50))} p95=${fmtMs(pct(times, 95))} p99=${fmtMs(pct(times, 99))}`,
  );
  return devices;
}
...
async function logStorm(devices, logsPerSecPerDevice, durationSec) {
  ...
  console.log(
    `  server RSS: ${before.rss}MB → ${after.rss}MB（Δ${(after.rss - before.rss).toFixed(1)}MB），事件循环利用率 ${after.elu.toFixed(1)}%`,
  );
      ...
      (_, i) =>
        new Promise((resolve, reject) => {
          ...
          ws.on("open", () => {
            ...
            const mine = deviceIds.slice(
              (i * perConsole) % deviceIds.length,
              ((i * perConsole) % deviceIds.length) + perConsole,
            );
...
async function execThroughput(deviceIds, concurrency, durationSec) {
  ...
  console.log(
    `  exec 延迟: p50=${fmtMs(s.p50)} p95=${fmtMs(s.p95)} p99=${fmtMs(s.p99)} max=${fmtMs(s.max)}`,
  );
...
async function httpPolling(durationSec) {
  ...
  const s = stats(latencies);
  console.log(
    `  轮询 ${latencies.length} 次: p50=${fmtMs(s.p50)} p95=${fmtMs(s.p95)} p99=${fmtMs(s.p99)}`,
  );
  return s;
}
...
try {
  ...
  if (!args.skipRamp) {
    ...
    for (const n of steps) {
      ...
      console.log(
        `  [爬坡 ${n}] 接入耗时 ${fmtMs(performance.now() - t0)}，server RSS ${s0.rss}→${(await serverStats()).rss}MB`,
      );
      /** 爬坡只测建连，测完立刻释放 */
  ...
  const consoleResult = await consoleFanout(
    pool.map((d) => d.id),
    M,
    N * LOG_RATE,
    D,
  ...
  console.log(
    `  控制台收到 log 广播 ${consoleResult.counter.log} 条（理论 ≈ ${(((storm2.sent * expectedPerConsole) / N) * M) | 0} = ${storm2.sent} × ${expectedPerConsole}/${N} × ${M}），其他消息 ${consoleResult.counter.other} 条`,
  );
  if (f0 && f1) {
    console.log(
      `  server 侧广播：sent Δ${f1.sent - f0.sent}，skippedClosed Δ${f1.skippedClosed - f0.skippedClosed}，skippedProject Δ${f1.skippedProject - f0.skippedProject}，backpressureClosed Δ${f1.backpressureClosed - f0.backpressureClosed}`,
    );
  }
