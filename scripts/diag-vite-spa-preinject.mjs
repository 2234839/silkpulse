...
async function execOn(devId, code) {
  ...
  try { return JSON.parse(j.result?.result ?? 'null') } catch { return j.result?.result }
}
...
for (let i = 0; i < 10; i++) {
  ...
  const devs = await (await fetch(`${SERVER}/api/devices`, { headers: { authorization: `Bearer ${KEY}` } })).json()
  dev = devs.devices?.find((x) => x.url?.includes('tab=devtools') && x.userAgent?.includes('HeadlessChrome'))
  if (dev) break
}
if (!dev) { console.log('❌ 设备未注册'); await b.close(); process.exit(1) }
...
if (healed) {
  ...
  console.log('tree 含 ConsoleApp/DevToolsPanel:', tj.includes('ConsoleApp') || tj.includes('App'), tj.slice(0, 120))
}
