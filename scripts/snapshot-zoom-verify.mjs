...
function setupHelpers(page) {
  return page.evaluateHandle(() => {
    const fracAt = (ax, ay) => {
      ...
      const lb = i.offsetLeft,
        tb = i.offsetTop; // offsetParent 内的布局位置不可靠，改用未变换盒
      // 未变换布局盒：rect 是变换后的。用 scale+translate 反解：
      // rect.center = layoutCenter + t （scale 绕布局中心进行）
      const dispW = b.width,
        dispH = b.height; // 显示尺寸 = layout * s
      const layoutW = dispW / s,
        layoutH = dispH / s;
...
async function main() {
  ...
  // 等 admin 会话就绪
  await con.waitForFunction(() => !document.body.textContent.includes("输入管理密钥"), {
    timeout: 8000,
  });

  // 等设备出现
  try {
    await con.waitForFunction(() => /在线设备 \(\d+\)/.test(document.body.textContent), {
      timeout: 15000,
    });
  } catch {
  ...
  await demo.goto(`${BASE}/demo?projectId=cs_playground`, { waitUntil: "networkidle2" });
  await sleep(2500);
  ...
  console.log(
    `含 clamp 往返漂移 = ${driftR2.toFixed(6)}（净变化步触底属预期，仅记录不作为断言失败项）`,
  );
  ...
  const after = await con.evaluate(() => {
    const m = new DOMMatrixReadOnly(getComputedStyle(img_q()).transform);
    function img_q() {
      return document.querySelector('img[alt="页面快照"]');
    }
    return { e: m.e, f: m.f };
  });

  const dx = after.e - before.e,
    dy = after.f - before.f;
  const dragged = Math.abs(dx) >= 55 && Math.abs(dy) >= 30;
  console.log(
    `offset Δ = (${dx.toFixed(1)}, ${dy.toFixed(1)}) px — ${dragged ? "✓ 容器空白区拖拽生效" : "✗ 未生效"}`,
  );
  ...
  console.log(
    "双击复位后 scale =",
    reset.scale.toFixed(4),
    reset.scale === 1 ? "✓" : "(若未复位需检查 dblclick 合成)",
  );
