...
async function waitDevice(titleHint, timeoutMs = 15000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const dev = (data.devices ?? []).find(
      (d) => !titleHint || (d.title ?? d.url ?? "").includes(titleHint),
    );
    if (dev) return dev;
    await sleep(500);
  }
...
async function main() {
  ...
  vueMounted.ready && vueMounted.hasVueAppAnchor && vueMounted.noVueHook
    ? ok(
        `生产 Vue 页面已启动（counter=${vueMounted.rendered}），无 devtools hook，DOM 锚点 __vue_app__ 在场`,
      )
  ...
  const setResult = await execOn(
    device,
    `__silkpulse_devtools_set(${idx}, 'state', ['count'], 100)`,
  );
  await sleep(300);
  const counterText = await page.evaluate(() =>
    document.querySelector(".counter")?.textContent?.trim(),
  );
  setResult?.success === true && counterText === "100"
    ? ok(`set 写 state 生效：count 42 → ${counterText}，UI 已响应式更新`)
    : fail(
        "set 后 UI 未更新",
        `result=${JSON.stringify(setResult.data).slice(0, 200)} counter=${counterText}`,
      );
